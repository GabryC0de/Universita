...
class Traduttore implements DictionaryUD {
    ...
    public void remove(Comparable key) { // realizzo linearSearch per cercare la chiave nell'array non ordinato
    ...
    public Object find(Comparable key) { // realizzo linearSearch per cercare la chiave nell'array non ordinato
        for (int i = 0; i < vSize; i++)
            // attenzione alla verifica dell'if: confronta chiavi, non coppie!
            if (v[i].getWord().compareTo((String) key) == 0)
    ...
    public void update(DictionaryUD newdict) { // Il codice della classe Traduttore ha accesso ai propri campi private
